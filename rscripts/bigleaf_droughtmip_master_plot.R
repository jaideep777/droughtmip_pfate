...
expt_dir = "GYF_ND13s3_500y_cont_evol_2"
expt_dir = "GYF_HB_10ky_evol_2"
...
dat_d = dat %>% mutate(date = as.POSIXct(i, orders = "%Y-%m-%d %H:%M:%S"))

dat = dat_d %>% group_by(year(date)) %>% summarize_all(mean)

dat %>% summarize(
  gpp = mean(assim_gross),
  swp = mean(swp),
  par = mean(ppfd_inst)
) %>% print()
...
dat_d %>% with(plot(assim_gross~swp, pch="."))
